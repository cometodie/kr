...
use Blogger\BlogBundle\Entity\bid;
...
class bidController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        if (!strcmp($role->getName(), "ROLE_ADMIN")){
            $bids = $em->getRepository('BloggerBlogBundle:bid')->getAll2();
//            $bids = $em->getRepository('BloggerBlogBundle:bid')->getBidOnIdUser($this->getUser()->getId());
        }else {
    ...
    {
        $bid = new Bid();
        $form = $this->createForm('Blogger\BlogBundle\Form\bidType', $bid);
    ...
    {
        $deleteForm = $this->createDeleteForm($bid);
        $editForm = $this->createForm('Blogger\BlogBundle\Form\bidType', $bid);
